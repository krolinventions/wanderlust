...
[CJDNS][] is actually the main inspiration for Wanderlust. However, we consider the routing algorithm (Kademlia without location swapping or changing connections) fundamentally broken. Wanderlust is an attempt to take the main idea (hashes of public keys as IPv6 addresses) and add working routing on top of it. This is done as a separate project as the CJDNS code is very much intertwined with the broken routing.
